...
function ConfigTab() {
  ...
  return (
    <section className="rounded-2xl border border-neutral-800 bg-neutral-900/40 p-6 space-y-3">
      ...
      <div className="flex items-center justify-between gap-3 mt-3 rounded border border-neutral-800 bg-neutral-950/60 px-3 py-2.5">
        <div className="min-w-0">
          <div className="text-xs text-neutral-300">Stale heartbeat check</div>
          <div className="text-[11px] text-neutral-500">
            When off, slots are never ended just because the extension stopped sending heartbeats. Idle / expired / quota / kill still apply.
          </div>
        </div>
        <button
          type="button"
          disabled={pending}
          onClick={() => update({ staleHeartbeatEnabled: !cfg.staleHeartbeatEnabled })}
          className={`shrink-0 inline-flex items-center gap-2 rounded px-2.5 py-1.5 text-xs border ${
        ...
        >
          <span
            className={`h-1.5 w-1.5 rounded-full ${
              cfg.staleHeartbeatEnabled ? "bg-emerald-400" : "bg-neutral-500"
            }`}
          />
          {cfg.staleHeartbeatEnabled ? "On" : "Off"}
        </button>
      </div>
      <div className="flex items-center justify-between gap-3 mt-3 rounded border border-neutral-800 bg-neutral-950/60 px-3 py-2.5">
        <div className="min-w-0">
          <div className="text-xs text-neutral-300">Bypass-permissions feature</div>
          <div className="text-[11px] text-neutral-500">
            When off, members cannot start a new timed bypass from the extension. Bypasses already running expire naturally.
          </div>
        </div>
        <button
          type="button"
          disabled={pending}
          onClick={() => update({ bypassPermissionsEnabled: !cfg.bypassPermissionsEnabled })}
          className={`shrink-0 inline-flex items-center gap-2 rounded px-2.5 py-1.5 text-xs border ${
            cfg.bypassPermissionsEnabled
              ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-300"
              : "border-neutral-700 bg-neutral-900 text-neutral-400"
          }`}
        >
          <span
            className={`h-1.5 w-1.5 rounded-full ${
              cfg.bypassPermissionsEnabled ? "bg-emerald-400" : "bg-neutral-500"
            }`}
          />
